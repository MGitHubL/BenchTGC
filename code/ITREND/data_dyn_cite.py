...
class DataHelper(Dataset):
    def __init__(self, args, file_path, node_feature_path, neg_size, hist_len, directed=False, transform=None, tlp_flag=False):
        self.node2hist = dict()
        ...
        node_emb = dict()
        with open(args.pre_feature_path, 'r') as reader:
            reader.readline()
            for line in reader:
        ...
        for i in range(len(node_emb)):
            self.pre_features.append(node_emb[i])
        self.pre_features = np.array(self.pre_features)
        ...
        if args.data == 'dblp' or args.data == 'brain':
            self.node_features = preprocessing.StandardScaler().fit_transform(self.node_features)
        else:
            self.node_features = preprocessing.StandardScaler().fit_transform(self.pre_features)
