...
class MNCIDataSet(Dataset):
    def __init__(self, file_path, neg_size, hist_len, feature_path, emb_size, directed=False, transform=None):
        ...
        with open(file_path, 'r') as infile:
            for line in infile:
                ...
                self.time_list.update([d_time])
                if s_node not in self.node_list:
                    self.node_list[s_node] = torch.zeros(emb_size)
                if t_node not in self.node_list:
                    self.node_list[t_node] = torch.zeros(emb_size)
